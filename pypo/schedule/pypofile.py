...
class PypoFile(Thread):
    ...
    def copy_file(self, media_item):
        ...
        is_remote = False
        if re.match('(?:http|https)://', src):
            is_remote = True
        
        if is_remote:
            self.logger.debug("File seems to be located on remote server: %s", src)
            self.copy_remote_file(media_item)
        else:
            self.logger.debug("File seems to be stored locally: %s", src)
            self.copy_local_file(media_item)
                
                
    def copy_local_file(self, media_item):
        """
    ...
    def copy_remote_file(self, media_item):
        ...
        if not dst_exists:
            print 'Not in cache - get it!: %s' % src
            file = urllib2.urlopen(src)
            output = open(dst,'wb')
            output.write(file.read())
            output.close()
        else:
            print 'Got file in cache!'
